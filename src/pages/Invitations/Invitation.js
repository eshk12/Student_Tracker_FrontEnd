...
import {getInvitations} from "../../microservices/invitations/invitations";
...
const Invitations = props => {
    ...
    useEffect(() => { //act like componentDidMount
        getInvitations().then((response) => {
            if (response.errorCode !== null && response.errorName !== null) {
                ...
                setInvitationList(response.object)
            }
        }).catch((e) => {
    ...
    const pageOptions = {
        sizePerPage: 3,
        totalSize: (!invitationList) ? 0 : invitationList.length,
        custom: true,
    }

    const handleTableChange = (type, {page, searchText}) => {
        setInvitationList(
            invitationList.filter(institute =>
                                                            ...
                                                            {toolkitProps => (
                                                                <React.Fragment>
                                                                    <Row className="row mb-2">
                                                                        <Col md={6}>
                                                                            <div className="mb-3">
                                                                                <Link to="/invitations/add"
                                                                                      className="btn btn-success waves-effect waves-light">
                                                                                    <i className="mdi mdi-plus me-2"></i>הוסף זימון
                                                                                </Link>
                                                                            </div>
                                                                        </Col>
